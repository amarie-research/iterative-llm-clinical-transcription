#!/usr/bin/env python3
"""
Qwen3-Next-80B Three-Pass Reversed Post-Processing for Transcription Correction
────────────────────────────────────────────────────────────────────────────────
Three-pass contextual correction using Qwen3-Next-80B (reversed order).

Pass 1: Transcription error correction
Pass 2: Speaker diarization (SPEAKER_XX -> actual role)
Pass 3: Transcription error correction refinement (second pass)

Features:
• Three-pass processing with correction-first approach
...
Usage:
    # Normal processing
    python src/post_processing/postprocess_qwen_80b_threepass_reversed.py
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "qwen_80b_threepass_reversed"
...
async def pass_diarization(prompt: str, corrected_text: str, sem: asyncio.Semaphore) -> str:
    """
    Pass 2: Speaker diarization (SPEAKER_XX -> actual role).
...
async def process_file(
    ...
    try:
        # Pass 1: Correction
        corrected = await pass_correction(correction_prompt, raw_text, req_sem, pass_num=1)

        # Pass 2: Diarization
        diarized = await pass_diarization(diarization_prompt, corrected, req_sem)

        # Pass 3: Correction refinement
        final = await pass_correction(correction_prompt, diarized, req_sem, pass_num=3)
...
def main():
    with BenchmarkTimer(
        script_name="qwen_80b_threepass_reversed",
        model_name=MODEL,
        approach="three-pass-reversed",
