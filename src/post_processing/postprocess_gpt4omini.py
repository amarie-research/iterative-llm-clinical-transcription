#!/usr/bin/env python3
"""
GPT-4o-mini Single-Pass Post-Processing for Transcription Correction
─────────────────────────────────────────────────────────────────────
Single-pass contextual correction using GPT-4o-mini.
...
Usage:
    # Normal processing
    python src/post_processing/postprocess_gpt4omini.py
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "gpt4omini"

MODEL: Final[str] = os.getenv("POSTPROCESS_MODEL", "gpt-4o-mini")
...
# Chunk size for large files (to avoid token limit)
# Will be reduced automatically on error (token limit, timeout, etc.)
...
CHUNK_SIZE_REDUCTION: Final[int] = 100  # Reduce by 100 on each error
...
async def call_gpt_text(prompt: str, segments_input: List[Tuple[str, str, str]], sem: asyncio.Semaphore) -> str:
    """
    Call GPT API with text-based output.
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if attempt == MAX_RETRIES:
                    raise RuntimeError(f"GPT API error after {MAX_RETRIES} retries: {exc}") from exc
...
async def process_chunks_with_size(
    ...
    if len(segments) <= chunk_size:
        ...
        result = await call_gpt_text(prompt, segments, sem)
    ...
    for i in range(0, len(segments), chunk_size):
        ...
        result = await call_gpt_text(prompt, chunk, sem)
...
async def process_chunks(
    ...
    """
    Process segments in chunks with automatic chunk size reduction on error.

    Starts with INITIAL_CHUNK_SIZE (500) and reduces by CHUNK_SIZE_REDUCTION (100)
    on each error (timeout, token limit, etc.), down to MIN_CHUNK_SIZE (100).
    """
    current_chunk_size = INITIAL_CHUNK_SIZE

    while current_chunk_size >= MIN_CHUNK_SIZE:
        try:
            return await process_chunks_with_size(prompt, segments, sem, current_chunk_size)
        except Exception as exc:
            error_str = str(exc).lower()
            # Detect recoverable errors: timeout or token limit reached
            is_recoverable = (
                "timeout" in error_str or
                "timed out" in error_str or
                "length limit" in error_str or
                "token" in error_str
            )

            if is_recoverable and current_chunk_size > MIN_CHUNK_SIZE:
                new_chunk_size = max(MIN_CHUNK_SIZE, current_chunk_size - CHUNK_SIZE_REDUCTION)
                print(f"    ⚠️ Error with chunk_size={current_chunk_size}, reducing to {new_chunk_size}...", flush=True)
                current_chunk_size = new_chunk_size
            else:
                # Not a recoverable error or already at minimum chunk size, re-raise
                raise

    # Should not reach here, but just in case
    return await process_chunks_with_size(prompt, segments, sem, MIN_CHUNK_SIZE)
...
async def process_file(
    path: Path, prompt: str, req_sem: asyncio.Semaphore, failures: list[str],
...
def main():
    with BenchmarkTimer(
        script_name="gpt4omini_singlepass",
        model_name=MODEL,
