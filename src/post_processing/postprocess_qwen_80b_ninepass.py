#!/usr/bin/env python3
"""
Qwen3-Next-80B Nine-Pass Post-Processing for Transcription Correction
───────────────────────────────────────────────────────────────────────
Nine-pass contextual correction using Qwen3-Next-80B (80B parameter model).
...
Pass 5: Speaker diarization refinement 2
Pass 6: Transcription error correction refinement 2
Pass 7: Speaker diarization refinement 3
Pass 8: Transcription error correction refinement 3
Pass 9: Speaker diarization final refinement

Features:
• Nine-pass processing (D-C-D-C-D-C-D-C-D)
...
Usage:
    # Normal processing
    python src/post_processing/postprocess_qwen_80b_ninepass.py
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "qwen_80b_ninepass"
...
async def process_file(
    ...
    try:
        ...
        pass4 = await pass_correction(correction_prompt, pass3, req_sem, pass_num=4)

        # Pass 5: Diarization refinement 2
        pass5 = await pass_diarization(diarization_prompt, pass4, req_sem, pass_num=5)

        # Pass 6: Correction refinement 2
        pass6 = await pass_correction(correction_prompt, pass5, req_sem, pass_num=6)

        # Pass 7: Diarization refinement 3
        pass7 = await pass_diarization(diarization_prompt, pass6, req_sem, pass_num=7)

        # Pass 8: Correction refinement 3
        pass8 = await pass_correction(correction_prompt, pass7, req_sem, pass_num=8)

        # Pass 9: Diarization final refinement
        final = await pass_diarization(diarization_prompt, pass8, req_sem, pass_num=9)
