#!/usr/bin/env python3
"""
Qwen3-Next-80B Two-Pass Post-Processing for Transcription Correction
─────────────────────────────────────────────────────────────────────
Two-pass contextual correction using Qwen3-Next-80B (80B parameter model).
...
Usage:
    # Normal processing
    python src/post_processing/postprocess_qwen_80b_twopass.py
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "qwen_80b_twopass"
...
async def pass1_diarization(prompt: str, raw: str, sem: asyncio.Semaphore) -> str:
    """
    Pass 1: Speaker diarization (SPEAKER_XX -> actual role).
    ...
    print(f"  -> Pass 1: Diarization ({len(segments)} segments)...", flush=True)
...
async def pass2_correction(prompt: str, diarized_text: str, sem: asyncio.Semaphore) -> str:
    """
    Pass 2: Transcription error correction.
...
async def process_file(
    ...
    try:
        # Pass 1: Diarization
        diarized = await pass1_diarization(diarization_prompt, raw_text, req_sem)

        # Pass 2: Correction
        corrected = await pass2_correction(correction_prompt, diarized, req_sem)
