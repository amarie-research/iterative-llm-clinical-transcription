#!/usr/bin/env python3
"""
Qwen3-VL-8B Single-Pass Post-Processing for Transcription Correction
─────────────────────────────────────────────────────────────────────
Single-pass contextual correction using Qwen3-VL-8B (8B parameter model).

Note: This is a vision model but we only use it for text processing.
...
Usage:
    # Normal processing
    python src/post_processing/postprocess_qwen_vl_8b.py
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "qwen_vl_8b"

# Qwen VL API configuration
MODEL: Final[str] = "Qwen/Qwen3-VL-8B-Instruct-FP8"
BASE_URL: Final[str] = "http://localhost:8001/v1"
...
# Will be reduced automatically on timeout
# Note: Qwen VL 8B is slower, so we start with 300 instead of 500
INITIAL_CHUNK_SIZE: Final[int] = int(os.getenv("POSTPROCESS_CHUNK_SIZE", "300"))
...
client = AsyncOpenAI(
    api_key=API_KEY,
    base_url=BASE_URL,
    timeout=600.0  # 10 minutes timeout for large models
)
...
async def process_chunks(
    ...
    on each timeout, down to MIN_CHUNK_SIZE (100).
    """
...
def main():
    with BenchmarkTimer(
        script_name="qwen_vl_8b_singlepass",
        model_name=MODEL,
