#!/usr/bin/env python3
"""
Qwen3-Next-80B Three-Pass Post-Processing for Transcription Correction
───────────────────────────────────────────────────────────────────────
Three-pass contextual correction using Qwen3-Next-80B (80B parameter model).

Pass 1: Speaker diarization (SPEAKER_XX -> actual role)
Pass 2: Transcription error correction
Pass 3: Speaker diarization refinement (second pass)

Features:
• Three-pass processing for better diarization results
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "qwen_80b_threepass"
...
async def pass_correction(prompt: str, diarized_text: str, sem: asyncio.Semaphore) -> str:
    """
    Pass 2: Transcription error correction.
    ...
    print(f"  -> Pass 2: Correction ({len(segments)} segments)...", flush=True)
...
async def process_file(
    ...
    try:
        # Pass 1: Diarization
        diarized = await pass_diarization(diarization_prompt, raw_text, req_sem, pass_num=1)

        # Pass 2: Correction
        corrected = await pass_correction(correction_prompt, diarized, req_sem)

        # Pass 3: Diarization refinement
        final = await pass_diarization(diarization_prompt, corrected, req_sem, pass_num=3)
...
def main():
    with BenchmarkTimer(
        script_name="qwen_80b_threepass",
        model_name=MODEL,
        approach="three-pass",
