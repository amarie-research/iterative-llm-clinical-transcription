#!/usr/bin/env python3
"""
Qwen3-Next-80B Eight-Pass Post-Processing for Transcription Correction
───────────────────────────────────────────────────────────────────────
Eight-pass contextual correction using Qwen3-Next-80B (80B parameter model).
...
Pass 8: Transcription error correction final

Features:
• Eight-pass processing (D-C-D-C-D-C-D-C)
...
Usage:
    # Normal processing
    python src/post_processing/postprocess_qwen_80b_eightpass.py
...
OUTPUT_ROOT: Final[Path] = PROJECT_ROOT / "results" / "post_processed" / "qwen_80b_eightpass"
...
async def process_file(
    ...
    try:
        ...
        # Pass 8: Correction final
        final = await pass_correction(correction_prompt, pass7, req_sem, pass_num=8)
...
def main():
    with BenchmarkTimer(
        script_name="qwen_80b_eightpass",
        model_name=MODEL,
        approach="eight-pass",
